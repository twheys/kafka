...
with_socket(Port, LFun, RFun) ->
    spawn(?MODULE, with_socket0, [Port, LFun, RFun]).

with_socket0(Port, LFun, RFun) ->
    process_flag(trap_exit, true),
    case gen_tcp:listen(Port, [binary, {packet, 0}, 
			       {reuseaddr, true},
			       {active, false}]) of
	{ok, LSock} ->
	    P = spawn_link(?MODULE, sock_handler, [self(), LFun, RFun, LSock]),
	    sock_loop(P, LFun, RFun, 1, nil, LSock);
	Other ->
	    Other
    end.
...
sock_loop(P, LFun, RFun, Procs, LastError, LSock) ->
    receive
	{P, connected} ->
	    P2 = spawn_link(?MODULE, sock_handler, [self(), LFun, RFun, LSock]),
	    sock_loop(P2, LFun, RFun, Procs+1, LastError, LSock);
	{'EXIT', P, {lasterror, Reason}} ->
	    P2 = spawn_link(?MODULE, sock_handler, [self(), LFun, RFun, LSock]),
	    sock_loop(P2, LFun, RFun, Procs, Reason, LSock);
	{'EXIT', _, {lasterror, Reason}} ->
	    sock_loop(P, LFun, RFun, Procs-1, Reason, LSock);
	{'EXIT', _, _} ->
	    sock_loop(P, LFun, RFun, Procs-1, LastError, LSock);
	{From, status} ->
	    From ! {self(), {ok, [{procs, Procs}, {lasterr, LastError}]}},
	    sock_loop(P, LFun, RFun, Procs, nil, LSock);
	{_From, stop} ->
	    exit(stop) %% propagate EXIT to all Pids
    end.


sock_handler(Top, LFun, RFun, S) ->
    case gen_tcp:accept(S) of
	{ok, Socket} ->
	    Top ! {self(), connected},
            R = spawn_link(?MODULE, sock_responder, [self(), RFun, S]),
	    hloop(Socket, LFun, R, list_to_binary([])) ;
	{error, Reason} ->
	    lasterror({noaccept, S, Reason})
    end.

sock_responder(Top, Fun, S) ->
    case Fun() of
        {ok, Msg} -> 
            gen_tcp:send(S, Msg),
            sock_responder(Top, Fun, S);
...
hloop(S, LFun, R, Ack) ->
	...
	{ok, B} when size(Ack) == 0 ->
	    process_data(S, LFun, R, B);
	{ok, B}  ->
	    process_data(S, LFun, R, erlang:concat_binary([B, Ack]));
	{error, Reason} ->
