...
send_system_msg(Client, Msg) when is_binary(Msg) -> goethe_messagerouter:send_mail(Client#client_state.id, {msg, {system_msg, Msg}});
...
process_admin_command(["info", User], Client, State) ->
    case get_user_by_nick(User, State) of
        ...
        {ok, #client_state{last_action=LastAction,nick=Nick,host=Host}} ->
            ...
            send_system_msg(Client, game_util:get_template("admin_info", Args))
    end,
    {ok, State};

% Admin Logout.
process_admin_command(["logout"], Client, State) ->
    goethe_messagerouter:broadcast_mail({msg, {admin_logged_out, Client#client_state.nick}}, []),
...
process_admin_command(["bans"], Client, State) ->
	...
	Args = [{bans, lists:map(fun(#user_ban{host=Host,reason=Reason,last_nick=Nick,until=Until}) -> [{host, Host}, {reason, Reason}, {nick, Nick}, {time, game_util:time_interval_str(Until)}] end, Bans)}],
	send_system_msg(Client, game_util:get_template("admin_bans", Args)),
	{ok, State};
...
process_command("auth", [?ADMIN_PASSWORD], Client, State) -> 
    ...
    case Client#client_state.admin of
        false -> goethe_messagerouter:broadcast_mail({msg, {admin_logged_in, Client#client_state.nick}}, []);
        _ -> ok
    end,
...
handle_call({join, {Nick, Host}}, _From, #state{clients=Clients} = State) when is_list(Nick) ->
    case {is_banned(Host,State), can_connect(Host), validate_nick(Nick, State)} of
        ...
        {_, {yes, NumConns}, {ok, ValidNick}} ->
            ...
            case goethe_messagerouter:create_mailbox(Session) of
                ok -> 
					update_host_conns(Host, NumConns + 1),
                    goethe_messagerouter:broadcast_mail({msg, {user_joined_room, ValidNick}}, [Session]),
...
handle_cast({get_msg_id, {Sess,Pid}}, State) ->
    case get_session(Sess, State) of
        ...
        {ok, C} -> 
            {_, NewState} = update_client(C, State),
            goethe_messagerouter:send_mail(Sess, {get_msg_id, Pid}), {noreply, NewState}
    end;
...
handle_cast({wait, {Sess, MsgID, Pid}}, State) when is_integer(MsgID) ->
    case get_session(Sess, State) of
        ...
        {ok, C} ->
            {_, NewState} = update_client(C, State), 
            goethe_messagerouter:send_mail(Sess, {add_listener, {MsgID, Pid}}), {noreply, NewState}
    end;
    
handle_cast({wait_finish, {Sess, Pid}}, State) ->
    case get_session(Sess, State) of
        {error, not_found} -> {noreply, State};
        {ok, _} -> goethe_messagerouter:send_mail(Sess, {remove_listener, Pid}), {noreply, State}
    end;
                ...
                _ ->
                    case should_rate_limit(NewC, NewState) of
                        {yes, {_, S2}} ->
                            ...
                            {noreply, S2};
                        {no, {RateClient, S2}} ->
                            goethe_messagerouter:broadcast_mail({msg, {chat_msg, {Nick, CleanMsg}}}, [ID]),
                            goethe_messagerouter:send_mail(ID, {msg, {sent_chat_msg, {Nick, CleanMsg}}}),
                ...
                _ ->
                    case should_rate_limit(NewC, NewState) of
                        ...
                        {no, {RateClient, S2}} ->
                            ...
                            goethe_messagerouter:broadcast_mail({msg, {chat_msg, {Nick, CleanMsg}}}, [SenderID]),
                            goethe_messagerouter:send_mail(RecipientID, {msg, {sent_chat_msg, {Nick, CleanMsg}}}),
                            goethe_messagerouter:send_mail(SenderID, {msg, {sent_chat_msg, {Nick, CleanMsg}}}),
                            {_, S3} = update_client(RateClient#client_state{last_msg=now()}, S2),
                            {noreply, S3}
                    end
            end
...
handle_cast({leave, {Sess, Reason}}, #state{clients=Clients} = State) when is_list(Reason) ->
    case get_session(Sess, State) of
        ...
        {ok, Client} ->
            ...
            goethe_messagerouter:delete_mailbox(Client#client_state.id),
            CleanReason =  game_util:unicode_clean(lists:sublist(Reason, 32)),
            goethe_messagerouter:broadcast_mail({msg, {user_left_room, {Client#client_state.nick, CleanReason}}}, [Client#client_state.id]),
