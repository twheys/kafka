...
start_link() -> mochiweb_http:start([{port, 34986}, {loop, {?MODULE, loop}}]).

% Request helpers
get_session(Req) -> web_util:get_parameter("chat_sess", Req:parse_cookie()).
...
wait(Req) ->
    ...
    game:wait_finish(get_session(Req), self()).
...
handle_request(Req, "/login/") ->
    Post = Req:parse_post(),
	case game:join(web_util:get_parameter("nick", Post), Req:get(peer)) of
	    {ok, SessID} -> 
