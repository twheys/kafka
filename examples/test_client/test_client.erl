...
process_inc(Conn, Binary) ->
    case read_json(Binary) of
	    ...
	    exit(normal);
	{more, Ack} ->
	    proc_lib:hibernate(?MODULE, read_pipe, {Conn, Ack});
	continue ->
	    proc_lib:hibernate(?MODULE, read_pipe, {Conn, list_to_binary([])});
	{error, Reason} ->
	    exit({user_error, Reason});
	Other ->
	    exit({bad_ret_from_fun, Other})
    end.
